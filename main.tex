...
\subsection{GHC 7.8~--- GHC 7.10}\label{710}
...
Для этого опишем функцию, которая принимает имя файла с исходным кодом и возвращает объект типа данных \textit{HsModule} (структура верхнего уровня для модуля). Схематично \textit{HsModule} представлен на рис.~\ref{hsmod}. 
\begin{figure}[h]
\begin{tikzpicture}[%
...
\subsection{Отфильтровать экземпляры класса Monad}\label{filtermonad}
...
\subsection{Генерация экземпляров классов Applicative и Functor}\label{generation}
...
\end{figure}
%$
\subsection{Фильтр экземпляров класса Monad}
...
\subsubsection{Генерация экземпляров}
...
Опишем основную функцию (\lstinline{main}) (см. рис.~\ref{main}). Она работает следующим образом: принимает имя файла в качестве аргумента командной строки, проводит его синтаксический анализ, применяет к нему функцию \lstinline{appendAllInstances} (является композицией вышеописанных функций), затем печатает в результат в этот же файл.
...
\end{minipage}
\end{figure}
Результаты соответствуют ожидаемым: программой были добавлены экземпляры классов \textit{Functor} и \textit{Applicative}. Реализация фукнций \lstinline{pure} и \lstinline{*>} отсутствует, поскольку в экземпляре монады соответствующие индентично работающие фукнции реализованы не были.
